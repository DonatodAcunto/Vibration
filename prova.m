%--------------------------------------------------------------------------
%   Calculating the natural frequencies of a beam              
...
% (1). assembly of K matrix
 ...
 for ii=1:2:matrix_size-3  
    ELNO=ELNO+1;
    [KE]=K_elemu(node(ELNO,2),node(ELNO+1,2),L,E,J);
...
% variables and were copied here  (DA CALCOLARE)
...
beta_ex= sqrt(modal_text_f*2*pi/sqrt(E*J/(rho*A*L^4)));
% auxiliary constants
  c0=sqrt(E/rho); 
   j=sqrt(J/A); 
  b2=beta.*beta;
...
%
  om=b2*c0*j/(L^2)/(2*pi);      % theoretical calculated angular frequencies [Hz]
  om_ex=b2_ex*c0*j/(L^2)/(2*pi); % modal test calculated angular frequencies [Hz]
%
% --------------------------------------------------------------------------
...
ix=1:6;
beta_fe=sqrt(f(ix)'*2*pi/sqrt(E*I/(pb*A*L^4)));
for  i=ix;
     modes(i) = beta(i)^2 * sqrt(E*I/(pb*A*L^4));
     modes(i) = modes(i)/(2*pi);
     modes_ex(i) = beta_ex(i)^2 * sqrt(E*I/(pb*A*L^4));
     modes_ex(i) = modes_ex(i)/(2*pi);
     modes_fe(i) = beta_fe(i)^2 * sqrt(E*I/(pb*A*L^4));
...
%}
% k_elemu
function [KE]=K_elemu(prenode,postnode,L,E,I)
...
%
  l=postnode-prenode;  
  L=l/L;
     % the length of present element per unit length, L:length_bar
  KE(1,1)=12     ;                
  KE(2,1)=-6*L  ; KE(2,2)=4*L^2 ; 
  KE(3,1)=-12    ; KE(3,2)=6*L   ; KE(3,3)=12     ; 
  KE(4,1)=-6*L  ; KE(4,2)=2*L^2 ; KE(4,3)=6*L   ; KE(4,4)=4*L^2;
  KE=KE/L^3*E*I/L^3;
for i=2:4
    for j=1:i-1
        KE(j,i)=KE(i,j);
...
% m_elemu
function [ME]=m_elemu(prenode,postnode,L,pb,A)
% PURPOSE : This is a subprogram as for Mass matrice
%
%  M=1/420[ 156  22*L   54     -13*L
%                4*L^2  13*L  -3*L^2  
%                        156    -22*L
%           symmetric            4*L^2];      
%
    l=postnode-prenode; 
    L=l/L;
       % the length of the present element per unit length, L:length_bar
    ME(1,1)=156    ;
    ME(2,1)=22*L  ; ME(2,2)=4*L^2 ; 
    ME(3,1)=54     ; ME(3,2)=13*L  ; ME(3,3)=156    ; 
    ME(4,1)=-13*L ; ME(4,2)=-3*L^2; ME(4,3)=-22*L ; ME(4,4)=4*L^2 ;
    ME=ME*L/420*pb*A*L;
    for i=2:4
      for j=1:i-1
          ME(j,i)=ME(i,j);
